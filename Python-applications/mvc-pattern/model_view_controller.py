...
# Na classe ModelBasic iremos 'empacotar' todas as operações CRUD
class ModelBasic(object):

    def __init__(self, application_items):
        self._item_type = 'product'  # atributo não público, só pode ser modificado por métodos da classe
        self.create_items(application_items)

    # método getter, para retornar o atributo não público '_item_type'
    @property
    def item_type(self):
        return self._item_type

    # método setter, para alterar o atributo não público '_item_type'
    # com isso, não é necessário ter métodos da classe para alterar esse atributo, estamos usando getter e setter
    @item_type.setter
...
# Na classe View não temos lógica, apenas métodos estáticos. Não há menção aos outros dois componentes do padrão MVC.
# Se você deseja criar uma interface do usuário sofisticada para seu aplicativo, basta substituir a Viewclasse.
class View(object):
...
"""
Agora que as regras e a lógica (o Model) e a representação da informação (a View) estão prontas, podemos nos concentrar
no Controller. Quando você instancia um Controller você tem que especificar um Model e uma View.

No entanto, isso é apenas composição , portanto, sempre que você quiser usar um modelo diferente e/ou uma exibição
diferente, basta conectá-los ao instanciar o controlador. O Controlador aceita as entradas do usuário e delega a
representação de dados para a Visualização e a manipulação de dados para o Modelo."""
