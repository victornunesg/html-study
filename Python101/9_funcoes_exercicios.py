...
- O % de stockout é dado por (Vendas Perdidas por Estoque) / (Vendas Concluídas + Vendas Perdidas por Estoque) -> essas
vendas são dadas em valor total (dinheiro) e não em quantidade de vendas
- Seu programa recebe um dicionário com todas as vendas da empresa, o status dela (se foi Concluída ou Cancelada) e,
...
- Para reforçar: As vendas Canceladas por qualquer outro motivo que não seja a Falta de Estoque não devem ser
...
#
#
# def stockout(lista_de_vendas):
#     perdas = 0
#     ganhos = 0
#     for item in lista_de_vendas:
#         valor, status, motivo = lista_de_vendas[item]
#         if 'Cancelada pelo Cliente' not in motivo:
#             if 'Concluído' in status:
#                 ganhos += valor
#             else:
#                 perdas += valor
#
#     return perdas / (perdas + ganhos)
#
#
# indicador = stockout(vendas)
# print(f'Indicador: {indicador:.2%}')

""" Item 2: Crie uma função para descobrir os clientes inadimplentes de uma empresa
O objetivo é identificar quem são os clientes inadimplentes e enviar essa lista de clientes para o setor de cobrança
poder fazer a cobrança dos clientes. Sua função deve então receber uma lista de clientes, analisar quais clientes estão
inadimplentes, e retornar uma lista com os clientes inadimplentes (apenas o CPF deles já é suficiente)
...
Isso significa que caso ou cliente esteja devendo 2.000 reais a 10 dias, ele não é inadimplente, pois não se passaram
20 dias ainda. Da mesma forma, se ele estiver devendo 500 reais por 40 dias, ele também não é inadimplente, dado que ele
deve menos de 1.000 reais.
...
#
#
# def inadimplencia(lista_clientes):
#     inadimplentes = []
#     for cliente in lista_clientes:
#         cpf, valor, dias = cliente  # fazendo o unpacking da tupla
#         if valor > 1000 and dias > 20:
#             inadimplentes.append(cpf)
#     return inadimplentes
#
#
# print(f'Clientes inadimplentes: {inadimplencia(clientes_devedores)}')

""" Cálculo do Percentual e da Lista de Vendedores

Queremos criar uma function que consiga identificar os vendedores que bateram uma meta, mas além disso, consigo já me
dar como resposta o cálculo do % da lista de vendedores que bateu a meta (para eu não precisar calcular manualmente)
Essa function deve receber 2 informações como parâmetro: a meta e um dicionário com os vendedores e suas vendas. E me
dar 2 respostas: uma lista com o nome dos vendedores que bateram a meta e o % de vendedores que bateu a meta.
"""
...
#
#
# def metabatida(lista_vendas, qtde_meta):
#     vendedores_meta = []
#     for vendedor, qtde in lista_vendas.items():
#         if qtde > qtde_meta:
#             vendedores_meta.append(vendedor)
#     percentual = len(vendedores_meta) / len(vendas)
#     return vendedores_meta, percentual
#
#
# print(metabatida(vendas, meta))
# nomes, percent = metabatida(vendas, meta)  # fazendo o unpacking
# print(nomes)
# print(f'{percent:.1%}')

""" APLICAÇÃO: Data Science e Inteligência Artificial

Quando criamos um modelo de previsão, precisamos treinar esse modelo e testar para ver se ele é um bom modelo ou não.
Temos então que pegar os nossos dados e dividir em 2 pedaços, uma lista de treino e uma lista de teste.
Vamos então pensar no exemplo de um modelo que tenta identificar qual o valor justo de um imóvel de acordo com o tamanho
do imóvel. Temos então 2 listas:

Lista 1: Preços Reais dos Imóveis
Lista 2: Tamanho do imóvel.

Vamos criar uma função que recebe 2 listas como entrada e que divide cada uma dessas listas em 2, um pedaço de treino e
um pedaço de teste.
...
#
#
# valores_treino, atributo_treino, valores_teste, atributo_teste = separar_listas(precos_imoveis, tamanho_imoveis)
# print(f'Valores treino: {valores_treino}\n'
#       f'Valores teste: {valores_teste}\n'
#       f'Tamanho treino: {atributo_treino}\n'
#       f'Tamanho teste: {atributo_teste}')
...
Isso significa que na prática, caímos em muitos casos de ter uma lista de produtos, mas quando olhamos por exemplo uma
base com a posição em estoque de cada produto, surge nessa "base" produtos que não existiam na nossa lista inicial.
...
Vamos criar uma function que pega todas essas listas e cria, a partir dela, uma lista de produtos. Com o objetivo de
saber todos os produtos que temos na empresa.

Repare que a function pode receber 1, 2, 3 ou quantas listas de estoque forem necessárias.
...
#
#
# lista_total = lista_estoque(estoque_fim_jan, estoque_fim_fev, estoque_fim_mar, estoque_fim_abr)
# print(f'Janeiro: {len(estoque_fim_jan)}')
# print(f'Fevereiro: {len(estoque_fim_fev)}')
